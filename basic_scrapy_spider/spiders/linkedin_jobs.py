...
class LinkedJobsSpider(scrapy.Spider):
    name = "linkedin_jobs"
    api_url = 'https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search?keywords=&location=&trk=public_jobs_jobs-search-bar_search-submit&start='
     

    def __init__(self, string1=None, string2=None, *args, **kwargs):
        super(LinkedJobsSpider, self).__init__(*args, **kwargs)
        if not string1:
            raise ValueError("Missing required argument: string1")
        if not string2:
            raise ValueError("Missing required argument: string2")
    ...
    def start_requests(self):
        self.api_url = f'https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search?keywords={self.string1}&location={self.string2}&trk=public_jobs_jobs-search-bar_search-submit&start='
        first_job_on_page = 0
